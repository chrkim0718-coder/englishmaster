...
export function UpdatePasswordFormAPI() {
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    try {
      console.log("🔄 Starting password update via API...")
      
      const response = await fetch('/api/auth/update-password-new', {
        method: 'POST',
        headers: {
      ...
      if (!contentType || !contentType.includes('application/json')) {
        ...
        setError("서버에서 잘못된 응답을 받았습니다. API 라우트를 확인해주세요.")
        setIsLoading(false)
        return
      }
      ...
      if (!response.ok) {
        ...
        setError(errorResult.error || "서버 오류가 발생했습니다.")
        setIsLoading(false)
        return
      }
      ...
      if (result.success) {
        ...
        setError(result.error || "비밀번호 업데이트에 실패했습니다.")
      }
    } catch (err) {
